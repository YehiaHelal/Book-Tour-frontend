...
export default function Profile() {
  ...
  async function handlegetpastorders() {
    const res = await axios.post(
      "https://book-tour-api-production.up.railway.app/api/orders/getuserorders",
      { message: "hello" },
      {
          ...
          <div className="user-profile-page-right">
            {showOrderback &&
              ...
              orderbackData.data.orders.map((item, index) => {
                return (
                  <div key={item._id}>
                    <p>Booking number: {index + 1} </p>
                    <p>Tour: {item.orderProducts.toString()}</p>
                    <p>
                      Numbers of Bookings for each tour:{" "}
                      {item.numberofitems.toString()}
                    </p>
                    <p>Booking Total Price: ${item.orderTotalValue}</p>
                    ------
                  </div>
                );
              })}
...
export const ProfileLoader = async ({}) => {
  const res = await axios.post(
    "https://book-tour-api-production.up.railway.app/api/users/profile",
    { message: "hello" },
    {
      withCredentials: true,
      headers: {
...
export const ProfileAction = async ({ request }) => {
  ...
  const datas = await axios.post(
    "https://book-tour-api-production.up.railway.app/api/users/updateinfo",
    {
      submission,
    },
    {
